// ==============================
// * TASK 3: draw two triangles with different shader program, where 2nd triangle is yellow
// ==============================
...
const char* fragment2_shader_path = "D:/dev/source/learn-opengl/HelloOpenGL-exercises/src/fragment2.glsl";
...
// * Compile fragment shader 2
	std::string fragment2_shader_source_code2 = shader_loader::ReadShaderSourceFromFile(fragment2_shader_path);
	const char* fragment2_shader_source_code_c_str = fragment2_shader_source_code2.c_str();

	unsigned fragment2_shader = glCreateShader(GL_FRAGMENT_SHADER);
	glShaderSource(fragment2_shader, 1, &fragment2_shader_source_code_c_str, NULL);
	std::cout << "[SHADER COMPILE INFO]: Start compile fragment shader..." << std::endl;
	glCompileShader(fragment2_shader);

	glGetShaderiv(fragment2_shader, GL_COMPILE_STATUS, &success);
	
	if (!success) {
		glGetShaderInfoLog(fragment2_shader, 512, NULL, info_log);
		std::cerr << "[SHADER COMPILE ERR]: " << info_log << std::endl;
	} else {
		std::cout << "[SHADER COMPILE INFO]: Shader compile successfuly" << std::endl;
	}
...
// * Link shaders 2
	...
	glAttachShader(shader_program2, fragment2_shader);
